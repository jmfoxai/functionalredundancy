...
function [F_weighted,F_raw_all,unsat_out,T,C] = Combined_Pathway_Solver_Lipid(init_cond,enz_conc,time_range)

%vector of model parameter values (fit values)
%p_vec = [a1 a2 a3 b1 c2 c1 c3 b2 b3 d1 d2 e f c4];
p_vec = [142473.7238 7597.676912 4.276689943 40213.92919 88.88525384 0.005388274 4.645634978 0.006677519 0.284982219 -0.285700283 3.348915642 2.886607673 15.90426225 2180.050007];
...
parameterized_Combined_Pathway_Model = @(t,c) Combined_Pathway_Model_Lipid(t,c,param_struct,opt_struct);
...
options = odeset('RelTol',1e-6,'AbsTol',1e-12,'MaxOrder',5,'Vectorized','on');
...
unsat = zeros(length(T),1);
for i = 1:5
    unsat(:) = unsat(:) + F_raw_all(:,2*(i+2));
end

unsat_out = unsat./sum(F_raw_all,2);%fraction unsaturated